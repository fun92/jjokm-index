...
메모 데이터는 `~/Library/Application Support/BeKkomIndex/memos.json`에 저장됩니다.

## 설치

GitHub Releases에서 최신 `.zip` 파일을 내려받은 뒤 압축을 풀고 `빼꼼 인덱스.app`을 `Applications` 폴더로 옮기면 됩니다.
...
- GitHub Releases에는 `release/빼꼼-인덱스-1.1.0.zip` 파일을 올리면 됩니다.
